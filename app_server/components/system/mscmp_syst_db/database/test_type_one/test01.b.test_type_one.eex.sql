...
ALTER TABLE ms_test.test_type_one OWNER TO <%= ms_owner %>;

REVOKE ALL ON TABLE ms_test.test_type_one FROM public;
GRANT ALL ON TABLE ms_test.test_type_one TO <%= ms_owner %>;

COMMENT ON
    TABLE ms_test.test_type_one IS
$DOC$A test table created by test01.b.test_type_one.eex.sql$DOC$;

COMMENT ON
    COLUMN ms_test.test_type_one.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN  ms_test.test_type_one.test_value IS
