...
# Location:    musebms/app_server/components/system/mscmp_syst_authn/test/network_rules_test.exs
...
defmodule NetworkRulesTest do
  ...
  describe "disallowed hosts" do
    test "Can determine if host is disallowed or not" do
      assert true == Impl.NetworkRules.host_disallowed?(~i"10.123.123.3")
      assert false == Impl.NetworkRules.host_disallowed?(~i"10.123.123.10")
    end

    test "Can create Disallowed Host" do
      assert false == Impl.NetworkRules.host_disallowed?(~i"10.122.122.1")
      assert {:ok, {:disallowed, _}} = Impl.NetworkRules.create_disallowed_host(~i"10.122.122.1")
      assert true == Impl.NetworkRules.host_disallowed?(~i"10.122.122.1")
    end

    test "Can delete Disallowed Host by record ID" do
      assert true == Impl.NetworkRules.host_disallowed?(~i"10.10.250.1")

      {:ok, {:disallowed, disallowed_host_record}} =
      ...
      assert false == Impl.NetworkRules.host_disallowed?(~i"10.10.250.1")

      assert {:error, :not_found} =
               Impl.NetworkRules.delete_disallowed_host(disallowed_host_record.id)
    end

    test "Can delete Disallowed Host by record" do
      assert true == Impl.NetworkRules.host_disallowed?(~i"10.10.250.2")

      {:ok, {:disallowed, disallowed_host_record}} =
        Impl.NetworkRules.get_disallowed_host_record_by_host(~i"10.10.250.2")

      assert :ok = Impl.NetworkRules.delete_disallowed_host(disallowed_host_record)
      assert false == Impl.NetworkRules.host_disallowed?(~i"10.10.250.2")

      assert {:error, :not_found} =
               Impl.NetworkRules.delete_disallowed_host(disallowed_host_record)
    end

    test "Can delete Disallowed Host by host IP address" do
      assert true == Impl.NetworkRules.host_disallowed?(~i"10.10.250.3")

      assert :ok = Impl.NetworkRules.delete_disallowed_host_addr(~i"10.10.250.3")
      assert false == Impl.NetworkRules.host_disallowed?(~i"10.10.250.3")

      assert {:error, :not_found} = Impl.NetworkRules.delete_disallowed_host_addr(~i"10.10.250.3")
    end

    test "Can retrieve Disallowed Host by record ID" do
      assert true == Impl.NetworkRules.host_disallowed?(~i"10.123.123.3")

      {:ok, {:disallowed, subject_rec}} =
  ...
  describe "applied network rules" do
    ...
    test "Can get Global Applied Network Rule" do
      ...
      assert {:ok, %{precedence: :global, functional_type: :deny}} =
    ...
    test "Can get Owner Applied Network Rule" do
      ...
      assert {:ok, %{precedence: :instance_owner, functional_type: :allow}} =
    ...
    test "Can get Instance Applied Network Rule" do
      ...
      assert {:ok, %{precedence: :instance, functional_type: :allow}} =
  ...
  describe "global network rules" do
    ...
    test "Can update Global Network Rule" do
      global_rule =
    ...
    test "Can get Global Network Rule" do
      global_rule =
    ...
    test "Can delete Global Network Rule" do
      global_rule_id =
  ...
  describe "owner network rules" do
    ...
    test "Can update Owner Network Rule" do
    ...
    test "Can get Owner Network Rule" do
      {:ok, owner_id} = MscmpSystInstance.get_owner_id_by_name("owner1")
    ...
    test "Can delete Owner Network Rule" do
      {:ok, owner_id} = MscmpSystInstance.get_owner_id_by_name("owner6")
  ...
  describe "instance network rules" do
    ...
    test "Can update Instance Network Rule" do
      {:ok, instance_id} =
    ...
    test "Can get Instance Network Rule" do
      {:ok, instance_id} =
        MscmpSystInstance.get_instance_id_by_name("app1_owner1_instance_types_std")
    ...
    test "Can delete Instance Network Rule" do
      {:ok, instance_id} =
