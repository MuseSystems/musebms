...
# Location:    musebms/app_server/components/system/mscmp_syst_authn/test/identity_api_token_test.exs
...
defmodule IdentityApiTokenTest do
  ...
  describe "API Token Creation" do
    test "Can create API Token Identity" do
      ...
      :ok = Impl.Identity.delete_identity(default_identity.id, "identity_types_sysdef_api")
      ...
      :ok = Impl.Identity.delete_identity(tokens_identity.id, "identity_types_sysdef_api")
      ...
      :ok = Impl.Identity.delete_identity(ext_name_identity.id, "identity_types_sysdef_api")
    end
  end
  ...
  describe "Access Account Identification" do
    test "Can identify Owned Access Account" do
      ...
      assert {:error, :not_found} =
               Impl.Identity.ApiToken.identify_access_account(target.account_identifier, nil)
    end
    ...
    test "Can identify Unowned Access Account" do
      ...
      assert {:error, :not_found} =
               Impl.Identity.ApiToken.identify_access_account(bad_target.account_identifier, nil)
    end
  end

  describe "External Name Management" do
    test "Can update Identity External Name by ID" do
      ...
      assert {:ok, %Msdata.SystIdentities{external_name: ^test_name}} =
