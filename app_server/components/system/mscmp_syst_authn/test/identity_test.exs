...
# Location:    musebms/app_server/components/system/mscmp_syst_authn/test/identity_test.exs
...
defmodule IdentityTest do
  ...
  describe "identity type operations" do
  ...
  describe "identity expiration management" do
    ...
    test "Can clear Identity expiration" do
      ...
      {:ok, %Msdata.SystIdentities{} = expired_identity} =
        Impl.Identity.Email.identify_access_account(
      ...
      assert {:ok, cleared_identity} =
               Impl.Identity.clear_identity_expiration(expired_identity.id)
    ...
    test "Can test if Identity is expired" do
      ...
      {:ok, %Msdata.SystIdentities{} = not_expired_identity} =
        Impl.Identity.Email.identify_access_account(
  ...
  describe "identity validation and deletion" do
    test "Can test if Identity is validated" do
      {:ok, owner_id} = MscmpSystInstance.get_owner_id_by_name("owner2")

      {:ok, %Msdata.SystIdentities{} = validated_identity} =
        Impl.Identity.Email.identify_access_account(
    ...
    test "Can delete Identity" do
      ...
      {:ok, %Msdata.SystIdentities{} = delete_identity} =
        Impl.Identity.Email.identify_access_account(
