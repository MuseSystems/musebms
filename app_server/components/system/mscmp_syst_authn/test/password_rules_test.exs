...
# Location:    musebms/app_server/components/system/mscmp_syst_authn/test/password_rules_test.exs
...
defmodule PasswordRulesTest do
  ...
  describe "disallowed password list management" do
    ...
    test "Can add Disallowed Password" do
      assert :ok =
               Impl.PasswordRules.create_disallowed_password("can_add_disallowed_password_test")
    ...
    test "Can delete Disallowed Password once" do
      ...
      assert :ok = Impl.PasswordRules.delete_disallowed_password("DeleteDisallowed!Test#02a")
  ...
  describe "global password rules" do
    test "Can retrieve Global Password Rules" do
      ...
      assert {:ok, second_pwd_rules} = Impl.PasswordRules.get_global_password_rules()
    ...
    test "Can update Global Password Rules" do
      ...
      assert :eq =
               DbTypes.compare(update_params.password_length, updated_pwd_rules.password_length)
      ...
      assert :eq =
               DbTypes.compare(revert_params.password_length, revert_pwd_rules.password_length)
  ...
  describe "owner password rules" do
    ...
    test "Can delete Owner Password Rules" do
      ...
      assert :ok = Impl.PasswordRules.delete_owner_password_rules(owner_id)

      assert {:error, :not_found} = Impl.PasswordRules.delete_owner_password_rules(owner_id)

      assert {:error, :not_found} = Impl.PasswordRules.get_owner_password_rules(owner_id)
  ...
  describe "effective password rules" do
