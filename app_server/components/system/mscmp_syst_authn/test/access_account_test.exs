...
# Location:    musebms/app_server/components/system/mscmp_syst_authn/test/access_account_test.exs
...
defmodule AccessAccountTest do
  ...
  describe "access account creation" do
    test "Can create an unowned Access Account" do
      ...
      assert {:ok, new_access_account} = Impl.AccessAccount.create_access_account(create_params)
  ...
  describe "access account state management" do
    ...
    test "Can retrieve functional type default Access Account State" do
      assert %Msdata.SystEnumItems{internal_name: "access_account_states_sysdef_inactive"} =
               Impl.AccessAccount.get_access_account_state_default(
                 :access_account_states_inactive
               )
    end
  end

  describe "access account updates and purging" do
    test "Can update an Access Account" do
      {:ok, target_account_id} =
        Impl.AccessAccount.get_access_account_id_by_name("update_test_accnt")
      ...
      assert {:ok, _updated_account} =
               Impl.AccessAccount.update_access_account(target_account_id, update_params)
    end

    test "Cannot update Access Account immutable values" do
      {:ok, target_account_id} =
        Impl.AccessAccount.get_access_account_id_by_name("bad_update_test_accnt")
    ...
    test "Can purge eligible Access Account" do
      {:ok, %Msdata.SystAccessAccounts{id: target_account_id}} =
        Impl.AccessAccount.get_access_account_by_name("purge_test_accnt")

      assert :ok =
               Impl.AccessAccount.purge_access_account(%Msdata.SystAccessAccounts{
    ...
    test "Cannot purge ineligible Access Account" do
      {:ok, %Msdata.SystAccessAccounts{id: target_account_id}} =
        Impl.AccessAccount.get_access_account_by_name("owned_all_access")
  ...
  describe "access account queries" do
    ...
    test "Can test if Access Accounts Exists" do
      ...
      assert false ==
               Impl.AccessAccount.access_account_id_exists?(
