...
defmodule MscmpSystSettings.Runtime.DevSupport do
  ...
  @default_dev_service_name :"MscmpSystSettings.DevSupportService"
  @default_test_service_name :"MscmpSystSettings.TestSupportService"
  ...
  devsupport_opts = [
    ...
    supervisor_name: [
      type: :atom,
      default: :"MscmpSystSettings.DevSupportSupervisor",
      doc: """
  ...
  testsupport_opts = [
    ...
    supervisor_name: [
      type: :atom,
      default: :"MscmpSystSettings.TestSupportSupervisor",
      doc: """
      DevSupport services are expected to be started under a Supervisor.  This
      option provides the name of the Supervisor to use.  If the Supervisor does
      not exist, a new Supervisor will be started with the given name.
  ...
  def start_support_services(opts) do
    ...
    _ =
      case Process.whereis(opts[:supervisor_name]) do
        nil ->
          Supervisor.start_link([settings_service_spec],
            name: opts[:supervisor_name],
            strategy: :one_for_one
          )

        supervisor_name ->
          Supervisor.start_child(supervisor_name, settings_service_spec)
      end
