-- File:        enum_types.sql
-- Location:    msbms/database/msbms_syst_data/tables/enum_types.sql
...
CREATE TABLE msbms_syst_data.enum_types
(
     id                      uuid        DEFAULT uuid_generate_v1( ) NOT NULL
        CONSTRAINT enum_types_pk PRIMARY KEY
    ,app_relation_id         uuid                                    NOT NULL
        CONSTRAINT enum_types_app_relation_id_fk
    ...
    ,app_feature_type_id     uuid                                    NOT NULL
        CONSTRAINT enum_types_app_feature_type_id_fk
...
REVOKE ALL ON TABLE msbms_syst_data.enum_types FROM public;
GRANT ALL ON TABLE msbms_syst_data.enum_types TO msbms_owner;

CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON msbms_syst_data.enum_types
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst_priv.trig_b_iu_set_diagnostic_columns();

COMMENT ON
    TABLE msbms_syst_data.enum_types IS
$DOC$A listing of 'types' for use by various system oriented relations.  These types
...
COMMENT ON
    COLUMN msbms_syst_data.enum_types.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON COLUMN
    msbms_syst_data.enum_types.app_relation_id IS
$DOC$Identifies the specific relation with which the enum_type record is associated.
This value forms part of a compound candidate key for the record along with the
enum_type.display_name column.$DOC$;

COMMENT ON COLUMN msbms_syst_data.enum_types.internal_name IS
...
COMMENT ON COLUMN msbms_syst_data.enum_types.display_name IS
...
COMMENT ON COLUMN msbms_syst_data.enum_types.sort_order IS
$DOC$A simple number based sort use to sort on-screen displays of the data in the
table.$DOC$;

COMMENT ON COLUMN msbms_syst_data.enum_types.description IS
$DOC$A text describing the meaning and use of the specific record.$DOC$;

COMMENT ON COLUMN msbms_syst_data.enum_types.app_feature_type_id IS
$DOC$A reference to the feature type to which the enumeration type is associated.$DOC$;

COMMENT ON COLUMN msbms_syst_data.enum_types.functional_type IS
$DOC$$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.enum_types.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.enum_types.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.enum_types.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.enum_types.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.enum_types.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.enum_types.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst_data.enum_types.diag_update_count IS
