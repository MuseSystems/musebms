-- File:        app_feature_types.sql
-- Location:    msbms/database/msbms_syst_data/tables/app_feature_types.sql
...
CREATE TABLE msbms_syst_data.app_feature_types
(
     id                      uuid        DEFAULT uuid_generate_v1( ) NOT NULL
        CONSTRAINT app_feature_types_pk PRIMARY KEY
    ,internal_name           text                                    NOT NULL
        CONSTRAINT app_feature_types_internal_name_udx UNIQUE
    ,display_name            text                                    NOT NULL
        CONSTRAINT app_feature_types_display_name_udx UNIQUE
...
ALTER TABLE msbms_syst_data.app_feature_types OWNER TO msbms_owner;

REVOKE ALL ON TABLE msbms_syst_data.app_feature_types FROM public;
GRANT ALL ON TABLE msbms_syst_data.app_feature_types TO msbms_owner;

CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON msbms_syst_data.app_feature_types
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst_priv.trig_b_iu_set_diagnostic_columns();

COMMENT ON
    TABLE msbms_syst_data.app_feature_types IS
$DOC$A list of the various types of forms, processes, and data managed by the ERP
system.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.app_feature_types.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.app_feature_types.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.app_feature_types.display_name IS
$DOC$A friendly name and candidate key for the record, suitable for use in user
app_functionals.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.app_feature_types.description IS
$DOC$A text describing the meaning and use of the specific record that may be
visible to users of the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.app_feature_types.feature_group IS
...
COMMENT ON
    COLUMN msbms_syst_data.app_feature_types.functional_type IS
...
COMMENT ON
    COLUMN msbms_syst_data.app_feature_types.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.app_feature_types.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.app_feature_types.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.app_feature_types.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.app_feature_types.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.app_feature_types.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst_data.app_feature_types.diag_update_count IS
...
COMMENT ON
    CONSTRAINT functional_type_chk
    ON msbms_syst_data.app_feature_types IS
