-- Source File: enum_person_types.eex.sql
-- Location:    msbms/database/instance/msbms_appl_data/tables/enum_person_types.eex.sql
...
CREATE TABLE msbms_appl_data.enum_person_types
(
     id                      uuid        DEFAULT uuid_generate_v1( ) NOT NULL
        CONSTRAINT enum_person_types_pk PRIMARY KEY
    ,internal_name           text                                    NOT NULL
        CONSTRAINT enum_person_types_internal_name_udx UNIQUE
...
ALTER TABLE msbms_appl_data.enum_person_types OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_appl_data.enum_person_types FROM public;
GRANT ALL ON TABLE msbms_appl_data.enum_person_types TO <%= msbms_owner %>;

CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON msbms_appl_data.enum_person_types
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst_priv.trig_b_iu_set_diagnostic_columns();

COMMENT ON
    TABLE msbms_appl_data.enum_person_types IS
$DOC$A list of the various types of entities may be represented by a person.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.enum_person_types.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.enum_person_types.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.enum_person_types.display_name IS
...
COMMENT ON
    COLUMN msbms_appl_data.enum_person_types.user_options IS
$DOC$Allows for user defined options related to the type similar to the way the
options field is envisioned.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.enum_person_types.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.enum_person_types.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.enum_person_types.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.enum_person_types.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.enum_person_types.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.enum_person_types.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_appl_data.enum_person_types.diag_update_count IS
