-- File:        conf_enums.eex.sql
-- Location:    database\common\msbms_syst_data\tables\conf_enums.eex.sql
...
CREATE TABLE msbms_syst_data.conf_enums
(
     id
        uuid
        NOT NULL DEFAULT uuid_generate_v1( )
        CONSTRAINT conf_enums_pk PRIMARY KEY
    ,internal_name
        text
        NOT NULL
        CONSTRAINT conf_enums_internal_name_udx UNIQUE
    ,display_name
        text
        NOT NULL
        CONSTRAINT conf_enums_display_name_udx UNIQUE
    ,syst_description
        text
        NOT NULL
    ,user_description
    ...
    ,feature_id
        uuid
        NOT NULL
        CONSTRAINT conf_enums_feature_id_fk
...
ALTER TABLE msbms_syst_data.conf_enums OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_syst_data.conf_enums FROM public;
GRANT ALL ON TABLE msbms_syst_data.conf_enums TO <%= msbms_owner %>;

CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON msbms_syst_data.conf_enums
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst_priv.trig_b_iu_set_diagnostic_columns();

COMMENT ON
    TABLE msbms_syst_data.conf_enums IS
...
COMMENT ON
    COLUMN msbms_syst_data.conf_enums.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.conf_enums.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.conf_enums.display_name IS
$DOC$A friendly name and candidate key for the record, suitable for use in user
interactions$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.conf_enums.syst_description IS
...
msbms_syst_data.conf_enums.user_description field instead.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.conf_enums.user_description IS
...
COMMENT ON
    COLUMN msbms_syst_data.conf_enums.feature_id IS
...
COMMENT ON
    COLUMN msbms_syst_data.conf_enums.syst_defined IS
...
COMMENT ON
    COLUMN msbms_syst_data.conf_enums.user_maintainable IS
...
COMMENT ON
    COLUMN msbms_syst_data.conf_enums.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.conf_enums.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record 
started.  This field will be the same as diag_timestamp_created for inserted 
records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.conf_enums.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the 
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.conf_enums.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.conf_enums.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst_data.conf_enums.diag_update_count IS
