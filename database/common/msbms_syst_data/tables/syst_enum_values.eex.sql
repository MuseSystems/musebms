...
-- Location:    database\common\msbms_syst_data\tables\syst_enum_values.eex.sql
...
CREATE TABLE msbms_syst_data.syst_enum_values
(
     id
        uuid
        NOT NULL DEFAULT uuid_generate_v1( )
        CONSTRAINT syst_enum_values_pk PRIMARY KEY
    ,internal_name
        text COLLATE msbms_syst_priv.variant_insensitive
        NOT NULL
        CONSTRAINT syst_enum_values_internal_name_udx UNIQUE
    ,display_name
        text COLLATE msbms_syst_priv.variant_insensitive
        NOT NULL
        CONSTRAINT syst_enum_values_display_name_udx UNIQUE
    ,external_name
        text
        NOT NULL
    ,enum_id
        uuid
        NOT NULL
        CONSTRAINT syst_enum_values_enum_fk
            REFERENCES msbms_syst_data.syst_enums (id) ON DELETE CASCADE
    ,functional_type_id
        uuid
        CONSTRAINT syst_enum_values_enum_functional_type_fk
            REFERENCES msbms_syst_data.syst_enum_functional_types (id)
    ,enum_default
        boolean
...
ALTER TABLE msbms_syst_data.syst_enum_values OWNER TO <%= msbms_owner %>;
...
CREATE CONSTRAINT TRIGGER a50_trig_a_i_syst_enum_values_check_functional_type
    ...
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst_priv.trig_a_syst_enum_values_check_functional_types();
...
CREATE CONSTRAINT TRIGGER a50_trig_a_u_syst_enum_values_check_functional_type
    ...
    FOR EACH ROW WHEN ( OLD.functional_type_id IS DISTINCT FROM NEW.functional_type_id )
        EXECUTE PROCEDURE msbms_syst_priv.trig_a_syst_enum_values_check_functional_types();
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.display_name IS
$DOC$A friendly name and candidate key for the record, suitable for use in user
interactions$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.external_name IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.enum_id IS
$DOC$The enumeration record with which the value is associated.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.functional_type_id IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.enum_default IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.functional_type_default IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.syst_defined IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.user_maintainable IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.syst_description IS
$DOC$A system default description describing the value and its use cases within the
enumeration.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.user_description IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.sort_order IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.syst_options IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.user_options IS
$DOC$Extended user defined options, similar to syst_options, but for the purpose of
driving custom functionality.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_values.diag_update_count IS
