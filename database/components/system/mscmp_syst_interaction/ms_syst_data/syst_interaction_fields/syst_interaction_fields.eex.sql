...
-- Location:    musebms/database/components/system/mscmp_syst_interaction/ms_syst_data/syst_interaction_fields/syst_interaction_fields.eex.sql
...
CREATE TABLE ms_syst_data.syst_interaction_fields
(
     id
        uuid
        NOT NULL DEFAULT uuidv7( )
        CONSTRAINT syst_interaction_fields_pk PRIMARY KEY
    ,interaction_context_id
        uuid
        NOT NULL
        CONSTRAINT syst_interaction_fields_interaction_contexts_fk
            REFERENCES ms_syst_data.syst_interaction_contexts ( id )
    ,internal_name
        text
        NOT NULL
        CONSTRAINT syst_interaction_fields_internal_name_udx UNIQUE
    ,perm_id
        uuid
        CONSTRAINT syst_interaction_fields_perms_fk
            REFERENCES ms_syst_data.syst_perms ( id )
    ,interaction_category_id
        uuid
        CONSTRAINT syst_interaction_fields_interaction_categories_fk
...
ALTER TABLE ms_syst_data.syst_interaction_fields OWNER TO <%= ms_owner %>;

REVOKE ALL ON TABLE ms_syst_data.syst_interaction_fields FROM public;
GRANT ALL ON TABLE ms_syst_data.syst_interaction_fields TO <%= ms_owner %>;

CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON ms_syst_data.syst_interaction_fields
...
DECLARE
    ...
    -- Columns
    v_interaction_context_id  ms_syst_priv.comments_config_table_column;
...
BEGIN
    ...
    v_comments_config.table_name   := 'syst_interaction_fields';

    v_comments_config.description :=
$DOC$Defines specific state fields which require a different designation of either
primary permission, categorical permission, or both as compared to Interaction
Context.$DOC$;
...
the parent Interaction Context record.$DOC$;
    ...
    v_interaction_context_id.description :=
$DOC$The parent Interaction Context to which this state field permission specification
belongs.$DOC$;

    v_perm_id.column_name := 'perm_id';
    v_perm_id.description :=
$DOC$Assigns a specific permission for the state field which overrides the default
primary permission otherwise inherited from the Interaction Context.$DOC$;
    v_perm_id.general_usage :=
