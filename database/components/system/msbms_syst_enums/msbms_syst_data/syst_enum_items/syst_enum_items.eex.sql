...
-- Location:    musebms/database/components/system/msbms_syst_enums/msbms_syst_data/syst_enum_items/syst_enum_items.eex.sql
...
CREATE TABLE msbms_syst_data.syst_enum_items
(
     id
        uuid
...
ALTER TABLE msbms_syst_data.syst_enum_items OWNER TO <%= msbms_owner %>;
...
CREATE TRIGGER b55_trig_a_iu_syst_enum_items_maintain_sort_order
    ...
    ON msbms_syst_data.syst_enum_items
    FOR EACH ROW
EXECUTE PROCEDURE msbms_syst_data.trig_a_iu_syst_enum_items_maintain_sort_order( );

COMMENT ON
    TABLE msbms_syst_data.syst_enum_items IS
$DOC$The list of values provided by an enumeration as well as related behavioral and
informational metadata.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.display_name IS
$DOC$A friendly name and candidate key for the record, suitable for use in user
interactions$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.external_name IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.enum_id IS
$DOC$The enumeration record with which the value is associated.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.functional_type_id IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.enum_default IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.functional_type_default IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.syst_defined IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.user_maintainable IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.syst_description IS
$DOC$A system default description describing the value and its use cases within the
enumeration.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.user_description IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.sort_order IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.syst_options IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.user_options IS
$DOC$Extended user defined options, similar to syst_options, but for the purpose of
driving custom functionality.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_enum_items.diag_update_count IS
