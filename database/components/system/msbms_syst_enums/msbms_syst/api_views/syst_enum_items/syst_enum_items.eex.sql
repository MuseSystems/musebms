...
-- Location:    musebms/database/components/system/msbms_syst_enums/msbms_syst/api_views/syst_enum_items/syst_enum_items.eex.sql
...
CREATE VIEW msbms_syst.syst_enum_items AS
    SELECT
        id
      , internal_name
      , display_name
...
ALTER VIEW msbms_syst.syst_enum_items OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_syst.syst_enum_items FROM PUBLIC;

CREATE TRIGGER a50_trig_i_i_syst_enum_items
    INSTEAD OF INSERT ON msbms_syst.syst_enum_items
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_i_syst_enum_items();

CREATE TRIGGER a50_trig_i_u_syst_enum_items
    INSTEAD OF UPDATE ON msbms_syst.syst_enum_items
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_u_syst_enum_items();

CREATE TRIGGER a50_trig_i_d_syst_enum_items
    INSTEAD OF DELETE ON msbms_syst.syst_enum_items
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_d_syst_enum_items();

COMMENT ON
    VIEW msbms_syst.syst_enum_items IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.id IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_enum_items.display_name IS
$DOC$A friendly name and candidate key for the record, suitable for use in user
interactions$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_enum_items.external_name IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.enum_id IS
$DOC$The enumeration record with which the value is associated.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_enum_items.functional_type_id IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.enum_default IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.functional_type_default IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.syst_defined IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.user_maintainable IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.syst_description IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.user_description IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.sort_order IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.syst_options IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.diag_role_created IS
$DOC$The database role which created the record.

Note that this column may not be updated via this API View.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_enum_items.diag_timestamp_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.diag_wallclock_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.diag_role_modified IS
$DOC$The database role which modified the record.

Note that this column may not be updated via this API View.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_enum_items.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_items.diag_update_count IS
