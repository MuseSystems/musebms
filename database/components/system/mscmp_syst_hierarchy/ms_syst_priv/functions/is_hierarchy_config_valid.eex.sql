...
-- Location:    musebms/database/components/system/mscmp_syst_hierarchy/ms_syst_priv/functions/is_hierarchy_config_valid.eex.sql
...
SELECT array_length(ms_syst_priv.hierarchy_config_check(p_hierarchy), 1) = 0;
...
CREATE OR REPLACE FUNCTION ms_syst_priv.is_hierarchy_config_valid(p_hierarchy_id uuid)
RETURNS boolean AS
$BODY$
...
SELECT array_length(ms_syst_priv.hierarchy_config_check(p_hierarchy_id), 1) = 0;

$BODY$
LANGUAGE sql STABLE;

ALTER FUNCTION ms_syst_priv.is_hierarchy_config_valid(p_hierarchy_id uuid)
    OWNER TO <%= ms_owner %>;
