...
CREATE OR REPLACE FUNCTION
    ms_syst_priv.hierarchy_config_check(p_hierarchy ms_syst_data.syst_hierarchies)
RETURNS text[] AS
$BODY$

-- File:        hierarchy_config_check.eex.sql
-- Location:    musebms/database/components/system/mscmp_syst_hierarchy/ms_syst_priv/functions/hierarchy_config_check.eex.sql
...
CREATE OR REPLACE FUNCTION ms_syst_priv.hierarchy_config_check(p_hierarchy_id uuid)
RETURNS text[] AS
$BODY$
...
SELECT ms_syst_priv.hierarchy_config_check( ch )
...
ALTER FUNCTION ms_syst_priv.hierarchy_config_check(p_hierarchy_id uuid)
    OWNER TO <%= ms_owner %>;

REVOKE EXECUTE ON FUNCTION
    ms_syst_priv.hierarchy_config_check(p_hierarchy_id uuid) FROM public;

GRANT EXECUTE ON FUNCTION
    ms_syst_priv.hierarchy_config_check(p_hierarchy_id uuid) TO <%= ms_owner %>;

COMMENT ON FUNCTION ms_syst_priv.hierarchy_config_check(p_hierarchy_id uuid) IS
$DOC$A convenience function allowing the `ms_syst_priv.hierarchy_config_check`
function to be called using a `ms_syst_data.syst_hierarchies.id` value.

Please see the `ms_syst_priv.hierarchy_config_check(ms_syst_data.syst_hierarchies)`
