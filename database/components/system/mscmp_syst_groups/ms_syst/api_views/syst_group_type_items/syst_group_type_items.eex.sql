...
-- Location:    musebms/database/components/system/mscmp_syst_groups/ms_syst/api_views/syst_group_type_items/syst_group_type_items.eex.sql
...
CREATE VIEW ms_syst.syst_group_type_items AS
SELECT
    id
  , internal_name
  , display_name
...
ALTER VIEW ms_syst.syst_group_type_items OWNER TO <%= ms_owner %>;

REVOKE ALL ON TABLE ms_syst.syst_group_type_items FROM PUBLIC;

CREATE TRIGGER a50_trig_i_i_syst_group_type_items
    INSTEAD OF INSERT ON ms_syst.syst_group_type_items
    FOR EACH ROW EXECUTE PROCEDURE ms_syst.trig_i_i_syst_group_type_items();

CREATE TRIGGER a50_trig_i_u_syst_group_type_items
    INSTEAD OF UPDATE ON ms_syst.syst_group_type_items
    FOR EACH ROW EXECUTE PROCEDURE ms_syst.trig_i_u_syst_group_type_items();

CREATE TRIGGER a50_trig_i_d_syst_group_type_items
    INSTEAD OF DELETE ON ms_syst.syst_group_type_items
    FOR EACH ROW EXECUTE PROCEDURE ms_syst.trig_i_d_syst_group_type_items();

COMMENT ON
    VIEW ms_syst.syst_group_type_items IS
...
COMMENT ON
    COLUMN ms_syst.syst_group_type_items.id IS
...
COMMENT ON
    COLUMN ms_syst.syst_group_type_items.internal_name IS
...
COMMENT ON
    COLUMN ms_syst.syst_group_type_items.display_name IS
...
COMMENT ON
    COLUMN ms_syst.syst_group_type_items.external_name IS
...
COMMENT ON
    COLUMN  ms_syst.syst_group_type_items.group_type_id IS
...
COMMENT ON
    COLUMN ms_syst.syst_group_type_items.hierarchy_depth IS
...
COMMENT ON
    COLUMN ms_syst.syst_group_type_items.diag_timestamp_created IS
...
COMMENT ON
    COLUMN ms_syst.syst_group_type_items.diag_role_created IS
$DOC$The database role which created the record.

This column is read only and not maintainable using this API view.$DOC$;

COMMENT ON
    COLUMN ms_syst.syst_group_type_items.diag_timestamp_modified IS
...
COMMENT ON
    COLUMN ms_syst.syst_group_type_items.diag_wallclock_modified IS
...
COMMENT ON
    COLUMN ms_syst.syst_group_type_items.diag_role_modified IS
$DOC$The database role which modified the record.

This column is read only and not maintainable using this API view.$DOC$;

COMMENT ON
    COLUMN ms_syst.syst_group_type_items.diag_row_version IS
...
COMMENT ON
    COLUMN ms_syst.syst_group_type_items.diag_update_count IS
