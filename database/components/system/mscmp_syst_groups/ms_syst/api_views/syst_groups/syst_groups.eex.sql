...
-- Location:    musebms/database/components/system/mscmp_syst_groups/ms_syst/api_views/syst_groups/syst_groups.eex.sql
...
CREATE VIEW ms_syst.syst_groups AS
SELECT
    id
  , internal_name
  , display_name
  ...
  , diag_row_version
  , diag_update_count
FROM ms_syst_data.syst_groups;

ALTER VIEW ms_syst.syst_groups OWNER TO <%= ms_owner %>;

REVOKE ALL ON TABLE ms_syst.syst_groups FROM PUBLIC;

CREATE TRIGGER a50_trig_i_i_syst_groups
    INSTEAD OF INSERT ON ms_syst.syst_groups
    FOR EACH ROW EXECUTE PROCEDURE ms_syst.trig_i_i_syst_groups();

CREATE TRIGGER a50_trig_i_u_syst_groups
    INSTEAD OF UPDATE ON ms_syst.syst_groups
    FOR EACH ROW EXECUTE PROCEDURE ms_syst.trig_i_u_syst_groups();

CREATE TRIGGER a50_trig_i_d_syst_groups
    INSTEAD OF DELETE ON ms_syst.syst_groups
    FOR EACH ROW EXECUTE PROCEDURE ms_syst.trig_i_d_syst_groups();

COMMENT ON
    VIEW ms_syst.syst_groups IS
...
COMMENT ON
    COLUMN ms_syst.syst_groups.id IS
...
COMMENT ON
    COLUMN ms_syst.syst_groups.internal_name IS
...
COMMENT ON
    COLUMN ms_syst.syst_groups.display_name IS
...
COMMENT ON
    COLUMN ms_syst.syst_groups.external_name IS
...
COMMENT ON
    COLUMN ms_syst.syst_groups.parent_group_id IS
...
COMMENT ON
    COLUMN ms_syst.syst_groups.group_type_item_id IS
...
COMMENT ON
    COLUMN ms_syst.syst_groups.user_maintainable IS
...
COMMENT ON
    COLUMN ms_syst.syst_groups.diag_role_created IS
$DOC$The database role which created the record.

This column is read only and not maintainable via this API view.$DOC$;

COMMENT ON
    COLUMN ms_syst.syst_groups.diag_timestamp_modified IS
...
COMMENT ON
    COLUMN ms_syst.syst_groups.diag_wallclock_modified IS
...
COMMENT ON
    COLUMN ms_syst.syst_groups.diag_role_modified IS
$DOC$The database role which modified the record.

This column is read only and not maintainable via this API view.$DOC$;

COMMENT ON
    COLUMN ms_syst.syst_groups.diag_row_version IS
...
COMMENT ON
    COLUMN ms_syst.syst_groups.diag_update_count IS
