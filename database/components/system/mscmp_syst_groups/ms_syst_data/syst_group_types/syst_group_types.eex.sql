...
-- Location:    musebms/database/components/system/mscmp_syst_groups/ms_syst_data/syst_group_types/syst_group_types.eex.sql
...
CREATE TABLE ms_syst_data.syst_group_types
(
     id
        uuid
        NOT NULL DEFAULT uuid_generate_v1( )
        CONSTRAINT syst_group_types_pk PRIMARY KEY
    ,internal_name
        text
        NOT NULL
        CONSTRAINT syst_group_types_internal_name_udx UNIQUE
    ,display_name
        text
        NOT NULL
        CONSTRAINT syst_group_types_display_name_udx UNIQUE
    ,functional_type_id
        uuid
        NOT NULL
...
ALTER TABLE ms_syst_data.syst_group_types OWNER TO <%= ms_owner %>;

REVOKE ALL ON TABLE ms_syst_data.syst_group_types FROM public;
GRANT ALL ON TABLE ms_syst_data.syst_group_types TO <%= ms_owner %>;

CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON ms_syst_data.syst_group_types
    FOR EACH ROW EXECUTE PROCEDURE ms_syst_priv.trig_b_iu_set_diagnostic_columns();

COMMENT ON
    TABLE ms_syst_data.syst_group_types IS
...
COMMENT ON
    COLUMN ms_syst_data.syst_group_types.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN ms_syst_data.syst_group_types.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN ms_syst_data.syst_group_types.display_name IS
$DOC$A friendly name and candidate key for the record, suitable for use in user
interactions$DOC$;

COMMENT ON
    COLUMN ms_syst_data.syst_group_types.functional_type_id IS
...
COMMENT ON
    COLUMN ms_syst_data.syst_group_types.syst_defined IS
...
COMMENT ON
    COLUMN ms_syst_data.syst_group_types.user_maintainable IS
...
COMMENT ON
    COLUMN ms_syst_data.syst_group_types.syst_description IS
...
COMMENT ON
    COLUMN ms_syst_data.syst_group_types.user_description IS
...
COMMENT ON
    COLUMN ms_syst_data.syst_group_types.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN ms_syst_data.syst_group_types.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN ms_syst_data.syst_group_types.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN ms_syst_data.syst_group_types.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN ms_syst_data.syst_group_types.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN ms_syst_data.syst_group_types.diag_row_version IS
...
COMMENT ON
    COLUMN ms_syst_data.syst_group_types.diag_update_count IS
