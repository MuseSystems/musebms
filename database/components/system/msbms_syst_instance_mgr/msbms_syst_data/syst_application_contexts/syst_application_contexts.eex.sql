...
-- Location:    musebms/database/components/system/msbms_syst_instance_mgr/msbms_syst_data/syst_application_contexts/syst_application_contexts.eex.sql
...
CREATE TABLE msbms_syst_data.syst_application_contexts
(
     id
        uuid
...
ALTER TABLE msbms_syst_data.syst_application_contexts OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_syst_data.syst_application_contexts FROM public;
GRANT ALL ON TABLE msbms_syst_data.syst_application_contexts TO <%= msbms_owner %>;

CREATE TRIGGER c50_trig_b_d_syst_application_contexts_validate_owner_context
    BEFORE DELETE ON msbms_syst_data.syst_application_contexts
    FOR EACH ROW
    WHEN (old.database_owner_context)
EXECUTE PROCEDURE msbms_syst_data.trig_b_iud_syst_application_contexts_validate_owner_context();

CREATE TRIGGER c50_trig_b_i_syst_application_contexts_validate_owner_context
    BEFORE INSERT ON msbms_syst_data.syst_application_contexts
    FOR EACH ROW
EXECUTE PROCEDURE msbms_syst_data.trig_b_iud_syst_application_contexts_validate_owner_context();

CREATE TRIGGER c50_trig_b_u_syst_application_contexts_validate_owner_context
    BEFORE UPDATE ON msbms_syst_data.syst_application_contexts
    FOR EACH ROW
    WHEN (new.database_owner_context AND old.database_owner_context != new.database_owner_context)
EXECUTE PROCEDURE msbms_syst_data.trig_b_iud_syst_application_contexts_validate_owner_context();

CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON msbms_syst_data.syst_application_contexts
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst_priv.trig_b_iu_set_diagnostic_columns();

COMMENT ON
    TABLE msbms_syst_data.syst_application_contexts IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.display_name IS
$DOC$A friendly name and candidate key for the record, suitable for use in user
interactions$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.application_id IS
$DOC$References the msbms_syst_data.syst_applications record which owns the
context.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.start_context IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.login_context IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.database_owner_context IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.description IS
$DOC$A user visible description of the application context, its role in the
application, uses, and any other helpful text.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_application_contexts.diag_update_count IS
