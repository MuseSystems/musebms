...
-- Location:    musebms/database/components/system/msbms_syst_instance_mgr/msbms_syst_data/syst_instance_type_applications/syst_instance_type_applications.eex.sql
...
CREATE TABLE msbms_syst_data.syst_instance_type_applications
(
     id
        uuid
...
ALTER TABLE msbms_syst_data.syst_instance_type_applications OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_syst_data.syst_instance_type_applications FROM public;
GRANT ALL ON TABLE msbms_syst_data.syst_instance_type_applications TO <%= msbms_owner %>;

CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON msbms_syst_data.syst_instance_type_applications
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst_priv.trig_b_iu_set_diagnostic_columns();

CREATE CONSTRAINT TRIGGER a50_trig_a_i_instance_types_enum_item_check
    AFTER INSERT ON msbms_syst_data.syst_instance_type_applications
...
CREATE CONSTRAINT TRIGGER a50_trig_a_u_instance_types_enum_item_check
    AFTER UPDATE ON msbms_syst_data.syst_instance_type_applications
...
CREATE TRIGGER b50_trig_a_i_syst_instance_type_apps_create_inst_type_contexts
    AFTER INSERT ON msbms_syst_data.syst_instance_type_applications
    FOR EACH ROW
        EXECUTE PROCEDURE msbms_syst_data.trig_a_i_syst_instance_type_apps_create_inst_type_contexts();

COMMENT ON
    TABLE msbms_syst_data.syst_instance_type_applications IS
