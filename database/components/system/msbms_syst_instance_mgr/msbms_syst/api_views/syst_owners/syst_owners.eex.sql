...
-- Location:    musebms/database/components/system/msbms_syst_instance_mgr/msbms_syst/api_views/syst_owners/syst_owners.eex.sql
...
COMMENT ON
    COLUMN msbms_syst.syst_owners.id IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owners.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_owners.display_name IS
$DOC$A friendly name and candidate key for the record, suitable for use in user
interactions$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_owners.diag_timestamp_created IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owners.diag_role_created IS
$DOC$The database role which created the record.

This column is read only and not maintainable via this API view.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_owners.diag_timestamp_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owners.diag_wallclock_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owners.diag_role_modified IS
$DOC$The database role which modified the record.

This column is read only and not maintainable via this API view.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_owners.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owners.diag_update_count IS
