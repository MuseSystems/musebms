...
BEGIN
    ...
    var_comments_config.table_name   := 'syst_actions';
...
$DOC$If the Action belongs to an Action Group, this column defines the owning Action
Group.$DOC$;
    var_action_group_id.general_usage :=
$DOC$If the Action is to be accessible via Command entry, it must define a parent
Action Group.  If the Action is not accessible via Commands, this value should
be left `NULL`.$DOC$;
...
$DOC$When the value of this column is `NULL`, the Action is not searchable or
