CREATE OR REPLACE FUNCTION msbms_syst_data.trig_a_d_syst_credentials_delete_identity()
RETURNS trigger AS
$BODY$

-- File:        trig_a_d_syst_credentials_delete_identity.eex.sql
-- Location:    musebms/database/components/system/msbms_syst_authentication/msbms_syst_data/syst_credentials/trig_a_d_syst_credentials_delete_identity.eex.sql
...
BEGIN
    ...
    DELETE FROM msbms_syst_data.syst_identities WHERE id = old.credential_for_identity_id;
...
ALTER FUNCTION msbms_syst_data.trig_a_d_syst_credentials_delete_identity()
    OWNER TO <%= msbms_owner %>;

REVOKE EXECUTE ON FUNCTION msbms_syst_data.trig_a_d_syst_credentials_delete_identity() FROM public;
GRANT EXECUTE ON FUNCTION msbms_syst_data.trig_a_d_syst_credentials_delete_identity() TO <%= msbms_owner %>;

COMMENT ON FUNCTION msbms_syst_data.trig_a_d_syst_credentials_delete_identity() IS
