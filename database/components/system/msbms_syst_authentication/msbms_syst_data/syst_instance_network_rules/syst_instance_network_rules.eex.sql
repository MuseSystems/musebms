...
-- Location:    musebms/database/components/system/msbms_syst_authentication/msbms_syst_data/syst_instance_network_rules/syst_instance_network_rules.eex.sql
...
CREATE TABLE msbms_syst_data.syst_instance_network_rules
(
     id
        uuid
...
ALTER TABLE msbms_syst_data.syst_instance_network_rules OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_syst_data.syst_instance_network_rules FROM public;
GRANT ALL ON TABLE msbms_syst_data.syst_instance_network_rules TO <%= msbms_owner %>;

CREATE TRIGGER b50_trig_a_iu_syst_instance_network_rule_ordering
    AFTER INSERT OR UPDATE ON msbms_syst_data.syst_instance_network_rules
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst_data.trig_a_iu_syst_instance_network_rule_ordering();

CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON msbms_syst_data.syst_instance_network_rules
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst_priv.trig_b_iu_set_diagnostic_columns();

COMMENT ON
    TABLE msbms_syst_data.syst_instance_network_rules IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_instance_network_rules.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN  msbms_syst_data.syst_instance_network_rules.instance_id IS
$DOC$The database identifier of the Instance record for whom the Network Rule is
being defined.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_instance_network_rules.ordering IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_instance_network_rules.functional_type IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_instance_network_rules.ip_host_or_network IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_instance_network_rules.ip_host_range_lower IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_instance_network_rules.ip_host_range_upper IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_instance_network_rules.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_instance_network_rules.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_instance_network_rules.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_instance_network_rules.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_instance_network_rules.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_instance_network_rules.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_instance_network_rules.diag_update_count IS
