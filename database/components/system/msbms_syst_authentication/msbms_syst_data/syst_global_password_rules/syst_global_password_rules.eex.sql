...
-- Location:    musebms/database/components/system/msbms_syst_authentication/msbms_syst_data/syst_global_password_rules/syst_global_password_rules.eex.sql
...
CREATE TABLE msbms_syst_data.syst_global_password_rules
(
     id
        uuid
...
ALTER TABLE msbms_syst_data.syst_global_password_rules OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_syst_data.syst_global_password_rules FROM public;
GRANT ALL ON TABLE msbms_syst_data.syst_global_password_rules TO <%= msbms_owner %>;

CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON msbms_syst_data.syst_global_password_rules
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst_priv.trig_b_iu_set_diagnostic_columns();

COMMENT ON
    TABLE msbms_syst_data.syst_global_password_rules IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.password_length IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.max_age IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.require_upper_case IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.disallow_recently_used IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.require_mfa IS
$DOC$When true, an approved multi-factor authentication method must be used in
addition to the password credential.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.allowed_mfa_types IS
$DOC$A array of the approved multi-factor authentication methods.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_global_password_rules.diag_update_count IS
