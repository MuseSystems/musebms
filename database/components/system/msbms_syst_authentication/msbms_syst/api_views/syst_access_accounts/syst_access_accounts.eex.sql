...
-- Location:    musebms/database/components/system/msbms_syst_authentication/msbms_syst/api_views/syst_access_accounts/syst_access_accounts.eex.sql
...
CREATE VIEW msbms_syst.syst_access_accounts AS
    SELECT
          id
        , internal_name
        , external_name
...
ALTER VIEW msbms_syst.syst_access_accounts OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_syst.syst_access_accounts FROM PUBLIC;

CREATE TRIGGER a50_trig_i_i_syst_access_accounts
    INSTEAD OF INSERT ON msbms_syst.syst_access_accounts
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_i_syst_access_accounts();

CREATE TRIGGER a50_trig_i_u_syst_access_accounts
    INSTEAD OF UPDATE ON msbms_syst.syst_access_accounts
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_u_syst_access_accounts();

CREATE TRIGGER a50_trig_i_d_syst_access_accounts
    INSTEAD OF DELETE ON msbms_syst.syst_access_accounts
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_d_syst_access_accounts();

COMMENT ON
    VIEW msbms_syst.syst_access_accounts IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_accounts.id IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_accounts.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_access_accounts.external_name IS
$DOC$Provides a user visible name for display purposes only.  This field is not
unique and may not be used as a key.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_access_accounts.owning_owner_id IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_accounts.allow_global_logins IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_accounts.diag_timestamp_created IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_accounts.diag_role_created IS
$DOC$The database role which created the record.

This value is read only from this API view.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_access_accounts.diag_timestamp_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_accounts.diag_wallclock_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_accounts.diag_role_modified IS
$DOC$The database role which modified the record.

This value is read only from this API view.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_access_accounts.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_accounts.diag_update_count IS
