...
-- Location:    musebms/database/components/system/msbms_syst_authentication/msbms_syst/api_views/syst_access_account_instance_assocs/syst_access_account_instance_assocs.eex.sql
...
CREATE VIEW msbms_syst.syst_access_account_instance_assocs AS
SELECT
    id
  , access_account_id
  , instance_id
...
ALTER VIEW msbms_syst.syst_access_account_instance_assocs OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_syst.syst_access_account_instance_assocs FROM PUBLIC;

CREATE TRIGGER a50_trig_i_i_syst_access_account_instance_assocs
    INSTEAD OF INSERT ON msbms_syst.syst_access_account_instance_assocs
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_i_syst_access_account_instance_assocs();

CREATE TRIGGER a50_trig_i_u_syst_access_account_instance_assocs
    INSTEAD OF UPDATE ON msbms_syst.syst_access_account_instance_assocs
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_u_syst_access_account_instance_assocs();

CREATE TRIGGER a50_trig_i_d_syst_access_account_instance_assocs
    INSTEAD OF DELETE ON msbms_syst.syst_access_account_instance_assocs
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_d_syst_access_account_instance_assocs();

COMMENT ON
    VIEW msbms_syst.syst_access_account_instance_assocs IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_account_instance_assocs.id IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_account_instance_assocs.instance_id IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_account_instance_assocs.access_granted IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_account_instance_assocs.invitation_issued IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_account_instance_assocs.invitation_expires IS
$DOC$The timestamp at which the invitation to access a given instance expires.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_access_account_instance_assocs.invitation_declined IS
$DOC$The timestamp at which the access account holder explicitly declined the
invitation to access the given instance.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_access_account_instance_assocs.diag_timestamp_created IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_account_instance_assocs.diag_role_created IS
$DOC$The database role which created the record.

This value is read only from this API view.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_access_account_instance_assocs.diag_timestamp_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_account_instance_assocs.diag_wallclock_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_account_instance_assocs.diag_role_modified IS
$DOC$The database role which modified the record.

This value is read only from this API view.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_access_account_instance_assocs.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst.syst_access_account_instance_assocs.diag_update_count IS
