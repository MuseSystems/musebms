...
-- Location:    musebms/database/components/system/msbms_syst_authentication/msbms_syst/api_views/syst_global_network_rules/syst_global_network_rules.eex.sql
...
ALTER VIEW msbms_syst.syst_global_network_rules OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_syst.syst_global_network_rules FROM PUBLIC;

CREATE TRIGGER a50_trig_i_i_syst_global_network_rules
    INSTEAD OF INSERT ON msbms_syst.syst_global_network_rules
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_i_syst_global_network_rules();

CREATE TRIGGER a50_trig_i_u_syst_global_network_rules
    INSTEAD OF UPDATE ON msbms_syst.syst_global_network_rules
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_u_syst_global_network_rules();

CREATE TRIGGER a50_trig_i_d_syst_global_network_rules
    INSTEAD OF DELETE ON msbms_syst.syst_global_network_rules
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_d_syst_global_network_rules();

COMMENT ON
    VIEW msbms_syst.syst_global_network_rules IS
...
COMMENT ON
    COLUMN msbms_syst.syst_global_network_rules.id IS
...
COMMENT ON
    COLUMN msbms_syst.syst_global_network_rules.ip_host_or_network IS
...
COMMENT ON
    COLUMN msbms_syst.syst_global_network_rules.ip_host_range_lower IS
...
COMMENT ON
    COLUMN msbms_syst.syst_global_network_rules.ip_host_range_upper IS
...
COMMENT ON
    COLUMN msbms_syst.syst_global_network_rules.ip_family IS
$DOC$
...
COMMENT ON
    COLUMN msbms_syst.syst_global_network_rules.diag_timestamp_created IS
...
COMMENT ON
    COLUMN msbms_syst.syst_global_network_rules.diag_role_created IS
$DOC$The database role which created the record.

This value is read only from this API view.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_global_network_rules.diag_timestamp_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_global_network_rules.diag_wallclock_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_global_network_rules.diag_role_modified IS
$DOC$The database role which modified the record.

This value is read only from this API view.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_global_network_rules.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst.syst_global_network_rules.diag_update_count IS
