...
-- Location:    musebms/database/components/system/msbms_syst_authentication/msbms_syst/api_views/syst_owner_password_rules/syst_owner_password_rules.eex.sql
...
CREATE VIEW msbms_syst.syst_owner_password_rules AS
SELECT
    id
  , owner_id
  , password_length
  ...
  , diag_row_version
  , diag_update_count
FROM msbms_syst_data.syst_owner_password_rules;

ALTER VIEW msbms_syst.syst_owner_password_rules OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_syst.syst_owner_password_rules FROM PUBLIC;

CREATE TRIGGER a50_trig_i_i_syst_owner_password_rules
    INSTEAD OF INSERT ON msbms_syst.syst_owner_password_rules
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_i_syst_owner_password_rules();

CREATE TRIGGER a50_trig_i_u_syst_owner_password_rules
    INSTEAD OF UPDATE ON msbms_syst.syst_owner_password_rules
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_u_syst_owner_password_rules();

CREATE TRIGGER a50_trig_i_d_syst_owner_password_rules
    INSTEAD OF DELETE ON msbms_syst.syst_owner_password_rules
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_d_syst_owner_password_rules();

COMMENT ON
    VIEW msbms_syst.syst_owner_password_rules IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.id IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.password_length IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.owner_id IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.max_age IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.require_upper_case IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.disallow_recently_used IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.disallow_known_compromised IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.require_mfa IS
$DOC$When true, an approved multi-factor authentication method must be used in
addition to the password credential.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.allowed_mfa_types IS
$DOC$A array of the approved multi-factor authentication methods.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.diag_timestamp_created IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.diag_role_created IS
$DOC$The database role which created the record.

This value is read only from this API view.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.diag_timestamp_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.diag_wallclock_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.diag_role_modified IS
$DOC$The database role which modified the record.

This value is read only from this API view.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst.syst_owner_password_rules.diag_update_count IS
