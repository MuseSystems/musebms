...
-- Location:    musebms/database/components/system/msbms_syst_numberings/msbms_syst_data/syst_numbering_segments/syst_numbering_segments.eex.sql
...
CREATE TABLE msbms_syst_data.syst_numbering_segments
(
     id
        uuid
    ...
    ,numbering_id
        ...
        CONSTRAINT syst_numbering_segments_numberings_fk
            REFERENCES msbms_syst_data.syst_numberings (id) ON DELETE CASCADE
    ,sort_order
        smallint
        NOT NULL DEFAULT 1
    ,numbering_segment_type_id
        ...
        CONSTRAINT syst_numbering_segments_numbering_segment_types_fk
            REFERENCES msbms_syst_data.syst_numbering_segment_types (id)
    ,display_length
        smallint
        NOT NULL
...
ALTER TABLE msbms_syst_data.syst_numbering_segments OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_syst_data.syst_numbering_segments FROM public;
GRANT ALL ON TABLE msbms_syst_data.syst_numbering_segments TO <%= msbms_owner %>;

CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON msbms_syst_data.syst_numbering_segments
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst_priv.trig_b_iu_set_diagnostic_columns();

COMMENT ON
    TABLE msbms_syst_data.syst_numbering_segments IS
$DOC$Defines the segments which make up a "numbering" in the application.  Segments $DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.sort_order IS
$DOC$Determines the positioning of the segment in the final number constructed from
all segments.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.numbering_segment_type_id IS
$DOC$References the syst_numbering_segment_types table to define the type of
segment represented by the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.display_length IS
$DOC$Sets the presentation length of sequence.  Numbering values that exceed the
display length will raise exceptions.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.use_padding IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.padding_side IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.padding_text IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.fixed_text IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.freetext_validator IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.enum_id IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.numbering_sequence_id IS
$DOC$If the segment is of a numeric sequence type, this column references the
sequence to be used in generating numbers.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_numbering_segments.diag_update_count IS
