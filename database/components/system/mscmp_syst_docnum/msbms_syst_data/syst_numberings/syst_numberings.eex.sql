...
-- Location:    musebms/database/components/system/msbms_syst_numberings/msbms_syst_data/syst_numberings/syst_numberings.eex.sql
...
CREATE TABLE msbms_syst_data.syst_numberings
(
     id
        uuid
...
ALTER TABLE msbms_syst_data.syst_numberings OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_syst_data.syst_numberings FROM public;
GRANT ALL ON TABLE msbms_syst_data.syst_numberings TO <%= msbms_owner %>;

CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON msbms_syst_data.syst_numberings
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst_priv.trig_b_iu_set_diagnostic_columns();

COMMENT ON
    TABLE msbms_syst_data.syst_numberings IS
$DOC$Records the available numbering sequences in the system.  These may be system
created or user created.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.display_name IS
$DOC$A friendly name and candidate key for the record, suitable for use in user
interactions$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.syst_description IS
...
msbms_syst_data.syst_numberings.user_description field instead.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.user_description IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.feature_id IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.syst_defined IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.user_maintainable IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst_data.syst_numberings.diag_update_count IS
