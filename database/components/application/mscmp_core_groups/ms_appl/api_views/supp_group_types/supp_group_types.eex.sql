...
-- Location:    musebms/database/components/application/mscmp_core_groups/ms_appl/api_views/supp_group_types/supp_group_types.eex.sql
...
CREATE VIEW ms_appl.supp_group_types AS
SELECT
    id
  , internal_name
  , display_name
...
ALTER VIEW ms_appl.supp_group_types OWNER TO <%= ms_owner %>;

REVOKE ALL ON TABLE ms_appl.supp_group_types FROM PUBLIC;

CREATE TRIGGER a50_trig_i_i_supp_group_types
    INSTEAD OF INSERT ON ms_appl.supp_group_types
    FOR EACH ROW EXECUTE PROCEDURE ms_appl.trig_i_i_supp_group_types();

CREATE TRIGGER a50_trig_i_u_supp_group_types
    INSTEAD OF UPDATE ON ms_appl.supp_group_types
    FOR EACH ROW EXECUTE PROCEDURE ms_appl.trig_i_u_supp_group_types();

CREATE TRIGGER a50_trig_i_d_supp_group_types
    INSTEAD OF DELETE ON ms_appl.supp_group_types
    FOR EACH ROW EXECUTE PROCEDURE ms_appl.trig_i_d_supp_group_types();

COMMENT ON
    VIEW ms_appl.supp_group_types IS
...
COMMENT ON
    COLUMN ms_appl.supp_group_types.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN ms_appl.supp_group_types.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN ms_appl.supp_group_types.display_name IS
$DOC$A friendly name and candidate key for the record, suitable for use in user
interactions$DOC$;

COMMENT ON
    COLUMN ms_appl.supp_group_types.functional_type_id IS
...
COMMENT ON
    COLUMN ms_appl.supp_group_types.syst_defined IS
...
COMMENT ON
    COLUMN ms_appl.supp_group_types.user_maintainable IS
...
COMMENT ON
    COLUMN ms_appl.supp_group_types.syst_description IS
...
COMMENT ON
    COLUMN ms_appl.supp_group_types.user_description IS
...
COMMENT ON
    COLUMN ms_appl.supp_group_types.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN ms_appl.supp_group_types.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN ms_appl.supp_group_types.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN ms_appl.supp_group_types.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN ms_appl.supp_group_types.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN ms_appl.supp_group_types.diag_row_version IS
...
COMMENT ON
    COLUMN ms_appl.supp_group_types.diag_update_count IS
