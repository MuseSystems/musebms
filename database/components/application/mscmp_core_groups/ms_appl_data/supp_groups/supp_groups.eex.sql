...
-- Location:    musebms/database/components/application/mscmp_core_groups/ms_appl_data/supp_groups/supp_groups.eex.sql
...
CREATE TABLE ms_appl_data.supp_groups
(
     id
        uuid
        NOT NULL DEFAULT uuid_generate_v1( )
        CONSTRAINT supp_groups_pk PRIMARY KEY
    ,internal_name
        text
        NOT NULL
        CONSTRAINT supp_groups_internal_name_udx UNIQUE
    ,display_name
        text
        NOT NULL
        CONSTRAINT supp_groups_display_name_udx UNIQUE
    ,external_name
        text
        NOT NULL
    ,parent_group_id
        uuid
        CONSTRAINT supp_group_parent_group_fk
            REFERENCES ms_appl_data.supp_groups ( id )
    ,group_type_item_id
        uuid
        NOT NULL
        CONSTRAINT supp_groups_group_type_item_fk
            REFERENCES ms_appl_data.supp_group_type_items ( id )
    ,syst_description
        text
        NOT NULL
    ,user_description
...
ALTER TABLE ms_appl_data.supp_groups OWNER TO <%= ms_owner %>;

REVOKE ALL ON TABLE ms_appl_data.supp_groups FROM public;
GRANT ALL ON TABLE ms_appl_data.supp_groups TO <%= ms_owner %>;

CREATE TRIGGER c50_trig_b_i_supp_groups_validate_parent_group_type_item
    BEFORE INSERT ON ms_appl_data.supp_groups
    FOR EACH ROW
    EXECUTE PROCEDURE ms_appl_data.trig_b_iu_supp_groups_validate_parent_group_type_item();

CREATE TRIGGER c50_trig_b_u_supp_groups_validate_parent_group_type_item
    BEFORE UPDATE ON ms_appl_data.supp_groups
    FOR EACH ROW
...
CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON ms_appl_data.supp_groups
    FOR EACH ROW EXECUTE PROCEDURE ms_syst_priv.trig_b_iu_set_diagnostic_columns();

COMMENT ON
    TABLE ms_appl_data.supp_groups IS
...
COMMENT ON
    COLUMN ms_appl_data.supp_groups.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.supp_groups.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.supp_groups.display_name IS
$DOC$A friendly name and candidate key for the record, suitable for use in user
interactions$DOC$;

COMMENT ON
    COLUMN ms_appl_data.supp_groups.external_name IS
...
COMMENT ON
    COLUMN ms_appl_data.supp_groups.parent_group_id IS
...
COMMENT ON
    COLUMN ms_appl_data.supp_groups.group_type_item_id IS
...
COMMENT ON
    COLUMN ms_appl_data.supp_groups.syst_description IS
...
COMMENT ON
    COLUMN ms_appl_data.supp_groups.user_description IS
...
COMMENT ON
    COLUMN ms_appl_data.supp_groups.syst_defined IS
...
COMMENT ON
    COLUMN ms_appl_data.supp_groups.user_maintainable IS
...
COMMENT ON
    COLUMN ms_appl_data.supp_groups.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record 
started.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.supp_groups.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.supp_groups.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record 
started.  This field will be the same as diag_timestamp_created for inserted 
records.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.supp_groups.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the 
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.supp_groups.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.supp_groups.diag_row_version IS
...
COMMENT ON
    COLUMN ms_appl_data.supp_groups.diag_update_count IS
