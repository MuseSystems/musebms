...
-- Location:    musebms/database/components/application/mscmp_core_hierarchy/ms_appl/api_views/conf_hierarchies/conf_hierarchies.eex.sql
...
CREATE VIEW ms_appl.conf_hierarchies AS
SELECT
    id
  , internal_name
  , display_name
...
ALTER VIEW ms_appl.conf_hierarchies OWNER TO <%= ms_owner %>;

REVOKE ALL ON TABLE ms_appl.conf_hierarchies FROM PUBLIC;

CREATE TRIGGER a50_trig_i_i_conf_hierarchies
    INSTEAD OF INSERT ON ms_appl.conf_hierarchies
    FOR EACH ROW EXECUTE PROCEDURE ms_appl.trig_i_i_conf_hierarchies();

CREATE TRIGGER a50_trig_i_u_conf_hierarchies
    INSTEAD OF UPDATE ON ms_appl.conf_hierarchies
    FOR EACH ROW EXECUTE PROCEDURE ms_appl.trig_i_u_conf_hierarchies();

CREATE TRIGGER a50_trig_i_d_conf_hierarchies
    INSTEAD OF DELETE ON ms_appl.conf_hierarchies
    FOR EACH ROW EXECUTE PROCEDURE ms_appl.trig_i_d_conf_hierarchies();

COMMENT ON
    VIEW ms_appl.conf_hierarchies IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.id IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.internal_name IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.display_name IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.hierarchy_type_id IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.hierarchy_state_id IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.syst_defined IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.user_maintainable IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.structured IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.syst_description IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.user_description IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.diag_timestamp_created IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.diag_role_created IS
$DOC$The database role which created the record.

This column is read only when using this API view.$DOC$;

COMMENT ON
    COLUMN ms_appl.conf_hierarchies.diag_timestamp_modified IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.diag_wallclock_modified IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.diag_role_modified IS
$DOC$The database role which modified the record.

This column is read only when using this API view.$DOC$;

COMMENT ON
    COLUMN ms_appl.conf_hierarchies.diag_row_version IS
...
COMMENT ON
    COLUMN ms_appl.conf_hierarchies.diag_update_count IS
