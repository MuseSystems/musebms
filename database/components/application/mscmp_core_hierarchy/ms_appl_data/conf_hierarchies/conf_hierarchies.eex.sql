-- File:        conf_hierarchies.eex.sql
-- Location:    musebms/database/components/application/mscmp_core_hierarchy/ms_appl_data/conf_hierarchies/conf_hierarchies.eex.sql
...
CREATE TABLE ms_appl_data.conf_hierarchies
(
     id
        uuid
        NOT NULL DEFAULT uuid_generate_v1( )
        CONSTRAINT conf_hierarchies_pk PRIMARY KEY
    ,internal_name
        text
        NOT NULL
        CONSTRAINT conf_hierarchies_internal_name_udx UNIQUE
    ,display_name
        text
        NOT NULL
        CONSTRAINT conf_hierarchies_display_name_udx UNIQUE
    ,hierarchy_type_id
        uuid
        NOT NULL
        CONSTRAINT conf_hierarchies_hierarchy_type_fk
            REFERENCES ms_syst_data.syst_enum_items ( id )
            ON DELETE CASCADE
    ,hierarchy_state_id
        uuid
        NOT NULL
        CONSTRAINT conf_hierarchies_hierarchy_state_fk
            REFERENCES ms_syst_data.syst_enum_items ( id )
            ON DELETE CASCADE
    ,syst_defined
        boolean
...
ALTER TABLE ms_appl_data.conf_hierarchies OWNER TO <%= ms_owner %>;

REVOKE ALL ON TABLE ms_appl_data.conf_hierarchies FROM public;
GRANT ALL ON TABLE ms_appl_data.conf_hierarchies TO <%= ms_owner %>;

CREATE TRIGGER c50_trig_b_i_conf_hierarchies_validate_inactive
    BEFORE INSERT ON ms_appl_data.conf_hierarchies
    FOR EACH ROW EXECUTE PROCEDURE ms_appl_data.trig_b_i_conf_hierarchies_validate_inactive();

CREATE TRIGGER c50_trig_b_u_conf_hierarchies_validate_state_change
    BEFORE UPDATE ON ms_appl_data.conf_hierarchies
    FOR EACH ROW WHEN ( old.hierarchy_state_id != new.hierarchy_state_id )
        EXECUTE PROCEDURE ms_appl_data.trig_b_u_conf_hierarchies_validate_state_change();

CREATE TRIGGER c55_trig_b_u_conf_hierarchies_validate_structured
    BEFORE UPDATE ON ms_appl_data.conf_hierarchies
    FOR EACH ROW WHEN ( old.structured != new.structured )
        EXECUTE PROCEDURE ms_appl_data.trig_b_u_conf_hierarchies_validate_structured();

CREATE TRIGGER c50_trig_b_d_conf_hierarchies_validate_prereqs
    BEFORE DELETE ON ms_appl_data.conf_hierarchies
    FOR EACH ROW EXECUTE PROCEDURE ms_appl_data.trig_b_d_conf_hierarchies_validate_prereqs();

CREATE TRIGGER z99_trig_b_iu_set_diagnostic_columns
    BEFORE INSERT OR UPDATE ON ms_appl_data.conf_hierarchies
    FOR EACH ROW EXECUTE PROCEDURE ms_syst_priv.trig_b_iu_set_diagnostic_columns();

CREATE CONSTRAINT TRIGGER a50_trig_a_i_hierarchy_states_enum_item_check
    AFTER INSERT ON ms_appl_data.conf_hierarchies
...
CREATE CONSTRAINT TRIGGER a50_trig_a_u_hierarchy_states_enum_item_check
    AFTER UPDATE ON ms_appl_data.conf_hierarchies
    FOR EACH ROW WHEN ( old.hierarchy_state_id != new.hierarchy_state_id)
        EXECUTE PROCEDURE
...
COMMENT ON
    TABLE ms_appl_data.conf_hierarchies IS
...
Hierarchy Items relation (`ms_appl_data.conf_hierarchy_items`)
...
COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.display_name IS
$DOC$A friendly name and candidate key for the record, suitable for use in user
interactions$DOC$;

COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.hierarchy_type_id IS
...
COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.hierarchy_state_id IS
...
COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.syst_defined IS
...
COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.user_maintainable IS
...
COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.structured IS
...
COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.syst_description IS
...
COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.user_description IS
...
COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.diag_row_version IS
...
COMMENT ON
    COLUMN ms_appl_data.conf_hierarchies.diag_update_count IS
