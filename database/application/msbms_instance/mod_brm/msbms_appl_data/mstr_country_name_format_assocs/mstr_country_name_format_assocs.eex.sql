...
-- Location:    musebms/database/application/msbms_instance/mod_brm/msbms_appl_data/mstr_country_name_format_assocs/mstr_country_name_format_assocs.eex.sql
...
REVOKE ALL ON TABLE msbms_appl_data.mstr_country_name_format_assocs FROM public;
GRANT ALL ON TABLE msbms_appl_data.mstr_country_name_format_assocs TO <%= msbms_owner %>;
...
COMMENT ON
    COLUMN msbms_appl_data.mstr_country_name_format_assocs.id IS
$DOC$The record's primary key.  The definitive identifier of the record in the
system.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.mstr_country_name_format_assocs.country_id IS
$DOC$Identifies the country with with the name format is being associated.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.mstr_country_name_format_assocs.name_format_id IS
$DOC$Identifies the name format which is to be associated with the country.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.mstr_country_name_format_assocs.is_default_for_country IS
...
COMMENT ON
    COLUMN msbms_appl_data.mstr_country_name_format_assocs.diag_timestamp_created IS
$DOC$The database server date/time when the transaction which created the record
started.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.mstr_country_name_format_assocs.diag_role_created IS
$DOC$The database role which created the record.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.mstr_country_name_format_assocs.diag_timestamp_modified IS
$DOC$The database server date/time when the transaction which modified the record
started.  This field will be the same as diag_timestamp_created for inserted
records.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.mstr_country_name_format_assocs.diag_wallclock_modified IS
$DOC$The database server date/time at the moment the record was actually modified.
For long running transactions this time may be significantly later than the
value of diag_timestamp_modified.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.mstr_country_name_format_assocs.diag_role_modified IS
$DOC$The database role which modified the record.$DOC$;

COMMENT ON
    COLUMN msbms_appl_data.mstr_country_name_format_assocs.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_appl_data.mstr_country_name_format_assocs.diag_update_count IS
