...
DECLARE
    v_source_comments       text;
    v_resolved_supplemental text;
    v_function_comments     text;
BEGIN
    ...
    v_resolved_supplemental :=
        E'**Supplemental Notes**\n\n' || p_supplemental;

    v_function_comments :=
        regexp_replace(
            coalesce(v_source_comments || E'\n', '') ||
            coalesce(v_resolved_supplemental || E'\n', ''),
            '[\n\r\f\u000B\u0085\u2028\u2029]{3,}',
            E'\n\n' );
...
DECLARE
    ...
    -- Parameters
    v_p_source_schema ms_syst_priv.comments_config_function_param;
    v_p_source_name   ms_syst_priv.comments_config_function_param;
    v_p_target_schema ms_syst_priv.comments_config_function_param;
    v_p_target_name   ms_syst_priv.comments_config_function_param;
    v_p_supplemental  ms_syst_priv.comments_config_function_param;
...
BEGIN
    ...
    v_comments_config.function_name   := 'generate_comments_copy_function';
...
Note that this function current doesn't work with overloaded functions.$DOC$;
    ...
    v_p_source_schema.param_name := 'p_source_schema';
    v_p_source_schema.description :=
$DOC$The name of the schema which hosts the function whose comments are to be copied.$DOC$;

    v_p_source_name.param_name := 'p_source_name';
    v_p_source_name.description :=
$DOC$The function name from which comments will be copied.$DOC$;

    v_p_target_schema.param_name := 'p_target_schema';
    v_p_target_schema.description :=
$DOC$The name of the schema which hosts the function which will receive the copied
comments.$DOC$;

    v_p_target_name.param_name := 'p_target_name';
    v_p_target_name.description :=
$DOC$The name of the function to which the copied comments will be applied.$DOC$;

    v_p_supplemental.param_name := 'p_supplemental';
    v_p_supplemental.required := FALSE;
    v_p_supplemental.description :=
