CREATE OR REPLACE FUNCTION
    msbms_syst_priv.nonstandard_decode(p_base integer, p_tokens text, p_value text)
RETURNS bigint AS
$BODY$

-- File:        nonstandard_decode.eex.sql
-- Location:    musebms/database/all/msbms_syst_priv/functions/nonstandard_decode.eex.sql
...
ALTER FUNCTION msbms_syst_priv.nonstandard_decode(p_base integer, p_tokens text, p_value text)
    OWNER TO <%= msbms_owner %>;

REVOKE EXECUTE ON FUNCTION
    msbms_syst_priv.nonstandard_decode(p_base integer, p_tokens text, p_value text) FROM public;
...
COMMENT ON
    FUNCTION msbms_syst_priv.nonstandard_decode(p_base integer, p_tokens text, p_value text) IS
