CREATE OR REPLACE FUNCTION msbms_syst_priv.get_random_string(
                                  p_length integer
...
-- Location:    musebms/database/all/msbms_syst_priv/functions/get_random_string.eex.sql
...
ALTER FUNCTION msbms_syst_priv.get_random_string(p_length integer, p_tokens text)
    OWNER TO <%= msbms_owner %>;

REVOKE EXECUTE ON FUNCTION
    msbms_syst_priv.get_random_string(p_length integer, p_tokens text) FROM public;

GRANT EXECUTE ON FUNCTION
    msbms_syst_priv.get_random_string(p_length integer, p_tokens text) TO <%= msbms_owner %>;

COMMENT ON FUNCTION msbms_syst_priv.get_random_string(p_length integer, p_tokens text) IS
