...
COMMENT ON
    COLUMN msbms_syst.syst_settings.config_flag IS
$DOC$A boolean configuration point.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_integer IS
$DOC$An integer configuration point$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_integer_range IS
$DOC$An integer range configuration point.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_decimal IS
$DOC$An decimal configuration point (not floating point).$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_decimal_range IS
$DOC$A decimal range configuration point.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_interval IS
$DOC$An interval configuration point.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_date IS
$DOC$A date configuation point.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_date_range IS
$DOC$A date range configuration point.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_time IS
$DOC$A time configuration point (without time zone).$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_timestamp IS
$DOC$A full datetime configuration point including time zone.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_timestamp_range IS
$DOC$A range of timestamps with time zone configuration points.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_json IS
$DOC$A JSON configuration point.  Note that duplicate keys at the same level are not
allowed.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_text IS
$DOC$A text configuration point.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_uuid IS
$DOC$A UUID configuration point.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_settings.config_blob IS
