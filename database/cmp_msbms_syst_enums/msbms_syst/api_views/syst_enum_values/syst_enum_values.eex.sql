-- File:        syst_enum_values.eex.sql
-- Location:    database\cmp_msbms_syst_enums\msbms_syst\api_views\syst_enum_values\syst_enum_values.eex.sql
...
CREATE VIEW msbms_syst.syst_enum_values AS
    SELECT
        id
      , internal_name
      , display_name
...
ALTER VIEW msbms_syst.syst_enum_values OWNER TO <%= msbms_owner %>;

REVOKE ALL ON TABLE msbms_syst.syst_enum_values FROM PUBLIC;

GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE msbms_syst.syst_enum_values TO <%= msbms_appusr %>;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE msbms_syst.syst_enum_values TO <%= msbms_apiusr %>;

CREATE TRIGGER a50_trig_i_i_syst_enum_values
    INSTEAD OF INSERT ON msbms_syst.syst_enum_values
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_i_syst_enum_values();

CREATE TRIGGER a50_trig_i_u_syst_enum_values
    INSTEAD OF UPDATE ON msbms_syst.syst_enum_values
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_u_syst_enum_values();

CREATE TRIGGER a50_trig_i_d_syst_enum_values
    INSTEAD OF DELETE ON msbms_syst.syst_enum_values
    FOR EACH ROW EXECUTE PROCEDURE msbms_syst.trig_i_d_syst_enum_values();

COMMENT ON
    VIEW msbms_syst.syst_enum_values IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.id IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.internal_name IS
$DOC$A candidate key useful for programmatic references to individual records.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_enum_values.display_name IS
$DOC$A friendly name and candidate key for the record, suitable for use in user
interactions$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_enum_values.external_name IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.enum_id IS
$DOC$The enumeration record with which the value is associated.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_enum_values.functional_type_id IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.enum_default IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.functional_type_default IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.syst_defined IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.user_maintainable IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.syst_description IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.user_description IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.sort_order IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.syst_options IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.diag_role_created IS
$DOC$The database role which created the record.

Note that this column may not be updated via this API View.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_enum_values.diag_timestamp_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.diag_wallclock_modified IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.diag_role_modified IS
$DOC$The database role which modified the record.

Note that this column may not be updated via this API View.$DOC$;

COMMENT ON
    COLUMN msbms_syst.syst_enum_values.diag_row_version IS
...
COMMENT ON
    COLUMN msbms_syst.syst_enum_values.diag_update_count IS
